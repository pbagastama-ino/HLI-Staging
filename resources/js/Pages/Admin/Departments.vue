<template>
  <Head title="Department Category Management - HLI Admin" />
  <AdminLayout 
  ...
  >
    <div class="mb-6">
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-xl font-semibold text-gray-800">Department Categories</h2>
        </div>
        <div class="flex space-x-3">
          <Link
            href="/hli-admin/departments/create"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <button @click="sortBy('code')" class="flex items-center space-x-1 hover:text-gray-700">
                  <span>Code</span>
                  <svg v-if="sortField === 'code'" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path v-if="sortDirection === 'asc'" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                    <path v-else d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" />
                  </svg>
                </button>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="department in departments.data" :key="department.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900 font-mono">{{ department.code }}</div>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div class="flex space-x-2">
                  ...
                  <button
                    ...
                    :class="[
                      'hover:text-gray-700',
                      department.is_active ? 'text-yellow-600' : 'text-green-600'
                    ]"
                    :title="department.is_active ? 'Deactivate' : 'Activate'"
                  >
                    <svg v-if="department.is_active" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const sortField = ref(props.filters?.sort_by || 'sort_order')
...
const toggleStatus = (department) => {
  if (confirm(`Are you sure you want to ${department.is_active ? 'deactivate' : 'activate'} this department?`)) {
    router.patch(`/hli-admin/departments/${department.id}/toggle-status`, {}, {
      onSuccess: () => {
...
const deleteDepartment = (department) => {
  ...
  if (confirm(`Are you sure you want to delete "${department.name}"? This action cannot be undone.`)) {
    router.delete(`/hli-admin/departments/${department.id}`, {
      onSuccess: () => {
        // The page will refresh with updated data
      }
    })
